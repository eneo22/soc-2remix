...
export const TrainingHub = () => {
  ...
  const [filter, setFilter] = useState<'all' | 'analysis' | 'linux' | 'response'>('all');
      ...
      <div className="min-h-screen bg-background px-4 pt-14 pb-10">
        ...
        <div className="max-w-3xl mx-auto">
          <button
            ...
            className="mb-4 font-mono text-xs text-muted-foreground hover:text-foreground transition-colors"
          >
            ← Retour au hub
          </button>
          <h2 className="mb-6 font-mono text-lg text-primary terminal-glow">{activeModule.title}</h2>

          {activeModule.content.type === 'quiz' && (
            <TrainingQuiz content={activeModule.content as QuizContent} onComplete={() => handleComplete(activeModule)} />
          )}
          {activeModule.content.type === 'terminal' && (
            <TrainingTerminal content={activeModule.content as TerminalContent} onComplete={() => handleComplete(activeModule)} />
          )}
          {activeModule.content.type === 'project' && (
            <TrainingProject content={activeModule.content as ProjectContent} onComplete={() => handleComplete(activeModule)} />
          )}
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-background px-4 pt-14 pb-10">
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="flex gap-2 mb-6 flex-wrap">
          {(['all', 'analysis', 'linux', 'response'] as const).map(f => (
            <button
              key={f}
              onClick={() => setFilter(f)}
            ...
            >
              {f === 'all' ? 'Tous' : f === 'analysis' ? '🔍 Analyse' : f === 'linux' ? '💻 Linux' : '🛡️ Réponse'}
            </button>
          ))}
        </div>
            ...
            return (
              <motion.button
                ...
                className={`w-full rounded-lg border p-4 text-left transition-all ${
                  ...
                  unlocked ? `${categoryColors[module.category]} hover:shadow-lg cursor-pointer` :
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <span className="text-lg">
                      {completed ? '✅' : !unlocked ? '🔒' : module.type === 'quiz' ? '📝' : module.type === 'terminal' ? '💻' : '🔧'}
                    </span>
                    <div>
                  ...
                  <div className="text-right">
                    ...
                    <p className="font-mono text-[10px] text-muted-foreground mt-1">
                      +{Object.values(module.xpReward).reduce((a, b) => a + (b || 0), 0)} XP
                    </p>
                  </div>
                </div>
                {!unlocked && module.requiredSkills.length > 0 && (
                  <p className="mt-2 text-[10px] text-muted-foreground italic">
                    Requiert : {module.requiredSkills.join(', ')}
                  </p>
                )}
              </motion.button>
            );
