...
export const Ch3Scene2_Physical = () => {
  ...
  const handleCommand = (cmd: string) => {
    ...
    if (trimmed === 'ping 192.168.1.10') {
      ...
      if (!fixed) { setPingDone(true); playSFX('error'); }
      else { addXP('technical', 40); playSFX('success'); setPhase('done'); }
      ...
      <div className="max-w-2xl w-full">
        ...
        {phase === 'intro' && (
          <div className="space-y-4">
            <DialogBox character="marcus">
              <p>"Commence par le bas. Vérifie si le serveur intranet répond."</p>
            </DialogBox>
        ...
        {(phase === 'terminal' || phase === 'fix') && (
          <div className="space-y-4">
            ...
            {pingDone && !ipaDone && (
              <DialogBox character="marcus">
                <p>"Destination unreachable. Vérifie l'état de l'interface avec <span className="text-primary">ip a</span>."</p>
              </DialogBox>
            )}
            {ipaDone && !fixed && (
              <DialogBox character="marcus">
                <p>"L'interface est DOWN. Active-la avec <span className="text-primary">sudo ip link set eth0 up</span>."</p>
              </DialogBox>
            )}
            {fixed && (
              <DialogBox character="marcus">
                <p>"Bien. Maintenant reteste avec <span className="text-primary">ping 192.168.1.10</span>."</p>
              </DialogBox>
            )}
        ...
        {phase === 'done' && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4 text-center">
            <DialogBox character="marcus">
              <p>"Si aucun paquet ne sort, on suspecte le câble, le switch ou l'IP mal configurée. Tu viens de résoudre un problème de <span className="text-primary font-bold">Couche 1/3</span>."</p>
            </DialogBox>
